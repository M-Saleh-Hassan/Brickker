                ...
                <div class="col-md-9">
                    <div class="box-tabs">
                        <ul class="nav nav-tabs">
                            ...
                            <li><a data-toggle="tab" href="#menu1">منتجاتي المفضلة</a></li>
                        ...
                        <div class="tab-content">
                                    ...
                                    @foreach (Auth::user()->favorites as $favorite)
                                        <div class="col-md-3">
                                            <div class="p-item relate-item">
                                                <div class="img-item">
                                                    <a href="{{route('ar.product.index', ['title_tag'=>$favorite->product->title_tag])}}" target="_blank">
                                                ...
                                                <div class="p-info">
                                                    ...
                                                    <div>
                                                        ...
                                                        <a href="{{route('ar.product.index', ['title_tag'=>$favorite->product->title_tag])}}"><i class="fa fa-shopping-cart"></i></a>
                                                        <span class="price-p">{{$favorite->product->current_price}} {{$active_currency->title_ar}}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>                                                                    
                            ...
                            ">
                                <form id="project_add_form" class="form-horizontal" method="POST" action="javascript:void(0)" enctype="multipart/form-data">
                                    @csrf
                                    <div class="form-group">
                                        <label  class="col-sm-2 control-label">عنوان المشروع</label>
                                        <div class="col-sm-10">
                                            <input type="text" class="form-control"  name="project_title" placeholder="عنوان المشروع" value="{{(Request::old('project_title'))}}">
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <div class="col-sm-10">
...
@section('custom-js')
    ...
    $('#project_add_form').on('submit', function(event){
        ...
        $.ajax({
            url:"{{ route('ar.project.save',['username_tag' => Auth::user()->username_tag]) }}",
            method:"POST",
    ...
    $('.projects-container').on('submit', '.project-edit-form', function(event){
        ...
        $.ajax({
            url:"{{ route('ar.project.update',['username_tag' => Auth::user()->username_tag]) }}",
            method:"POST",
            data:serialized_form,
            dataType:'JSON',
            beforeSend: function(){
