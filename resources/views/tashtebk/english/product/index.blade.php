...
<main class="gray relative">
    ...
    <div class="container">
        <ul class="breadcrumb">
            ...
            <li><a href="{{route('en.category.show_all',['category'=>$product->category->title])}}">{{$product->category->title}}</a></li>
            <li>{{$product->title}}</li>
        </ul>
    </div>
    </section>
            ...
            <div class="row" style="margin-bottom:30px;">
                    ...
                    <div style="text-align:center;">
                        <div class="magnify">
                            ...
                            <div class="product__price-tag">
                              <p class="product__price-tag-price">{{$product->current_price}} {{$active_currency->title_en}}</p>
                            </div>
                            @endif
                        </div>
                ...
                <div class="col-md-7 product-right-details">
                    <div class="desc">
                        ...
                        <h4>{{$product->title}}</h4>
                        <!--<span>by :</span>--> <a class="saler" href="{{route('en.profile.show', [$product->user->username_tag])}}" target="_blank" title="By"> <i  class="fa fa-user"></i> {{$product->user->real_name}}</a>

                        <p class="pric @if($product->discount > 0) line-through @endif">{{$product->price}} {{$active_currency->title_en}}/{{$product->unit->title}}</p>
                        ...
                        @if(Auth::check())
                            <a href="#" class="btn-sec add-to-cart" >ADD TO Cart </a>
                        @else
                            <a href="#" class="btn-sec add-to-cart" type="button" data-toggle="modal" data-target="#Login-Modal">ADD TO Cart </a>
                        @endif
                        <ul class="list-unstyled">
                            <!--<li > <i class="fa fa-plus"></i> Handling Charge : 0.00</li>-->
                            <li><a href="https://www.logic-host.com/~tashtebak/tashtebat/public/policy/en"> <i  class="fa fa-check"></i> Return Policy </a></li>
                            @auth<li><a href="" type="button"   data-toggle="modal" data-target="#spottedmistake" data-content="#spottedmistake"><i  class="fa fa-comment-o"></i> spotted a mistake : let us know</a></li>@endauth
                        </ul>
                    </div>
                </div>
        ...
        <div class="container">
            <div class=" nav-review">
                <ul class="nav nav-tabs">
                    <li class="active"><a data-toggle="tab"  href="#spec">About Product</a></li>
                    <li><a data-toggle="tab"  href="#desc">Specification</a></li>
              ...
              <div class="tab-content">
                <div id="spec" class="tab-pane fade in active">
                    <h3>About Product</h3>
                    <ul class="list-unstyled ">
                        <li><span class="col-md-3 text-left title-bold">Brand</span><span class="col-md-9 text-left">{{$product->brand}}</span></li>
                        <li><span class="col-md-3 text-left title-bold">Model Name</span> <span class="col-md-9 text-left">{{$product->model_name}}</span></li>
                        ...
                        <li>
                            <span class="col-md-3 text-left title-bold">product Grade</span> <span class="col-md-9 text-left">{{$product->grade}}</span>
                        </li>
                    </ul>
                </div>
                <div id="desc" class="tab-pane fade">
                  <h3>Specification</h3>
                  <p class="desc" style="white-space: pre-line">{{$product->long_description}}</p>
                </div>
                      ...
                      <div class="row">
                          <div class="col-md-4">
                              <div class="rate text-center">
                                  ...
                                  <p class="out">{{$product->average_rating}} out of 5</p>
                          ...
                          <div class="col-md-4">
                                @for($i=5; $i> 0; $i--)
                                <div class="row">
                                    <div class="col-md-3 text-center" style="padding:0"><span class="progress-left">{{$i}} stars</span></div>
                          ...
                          <div class="col-md-4">
                              <div class="rate-modal">
                                  <p>{{$product->getRecommendedPercentage()}}% of users recommend this product to a friend.</p>
                      ...
                      <div class="row">
                          <ul class="list-unstyled customers-reviews">
                              ...
                              <li class="review-box">
                                  ...
                                  <p>By <span class="boldd">{{$review->user->username}} </span> on {{date("d M Y", strtotime($review->created_at))}}</p>
                                  <p>{{$review->review}}</p>
                                  <h5>WHAT'S GOOD ABOUT THIS PRODUCT:</h5>
                                  <p>{{$review->good}}</p>
                                  <h5>WHAT'S NOT SO GOOD ABOUT THIS PRODUCT:</h5>
                                  <p>{{$review->bad}}</p>
                                  <a class="helpful helpful-button" data-review-id="{{$review->id}}"><i class=" fa fa-thumbs-up"></i> Helpful({{$review->getHelpful(1)}})</a>
                                  <a class="helpful no not-helpful-button" data-review-id="{{$review->id}}"><i class=" fa fa-thumbs-down"></i> Not Helpful({{$review->getHelpful(0)}})</a>
                              </li>
                              @endforeach
                          </ul>
                      </div>
                       ...
                       <div class="p-item">
                            ...
                            <div class="p-info">
                                ...
                                <div>
                                    ...
                                    <a href="{{route('en.product.index', ['title_tag'=>$related->title_tag])}}"><i class="fa fa-shopping-cart"></i></a>
                                    <span class="price-p">{{$related->current_price}} {{$active_currency->title_en}}</span>
                                </div>
                            </div>
                        </div>
                      </div>
	...
	<div class="modal-dialog extend-modal-width" role="document">
	  <div class="modal-content">
		<div class="modal-header project-header">
		  ...
		  <h4 class="modal-title" id="myModalLabel">spotted mistake</h4>
		</div>
		<div class="modal-body">
			<form class="project-form" method="POST"  action="{{route('en.report.send')}}" enctype="multipart/form-data">
			    @csrf
                <div class="form-group col-md-12 text-center col">
                        <input type="text" name="message" style="width:95%" placeholder="enter your message" required>
...
@if(Auth::check())
    $('.product-right-details').on('click', '.add-to-cart', function(){
        ...
        $.ajax({
            url:"{{ route('en.checkout.order.add', [$product->id, Auth::user()->username_tag]) }}",
            method:"POST",
    ...
    $('.checkout-modal-body').on('click', '.delete-order', function(){
        if(confirm('Are you sure you want to Delete ?'))
        {
            ...
            $.ajax({
                url:"{{ route('en.checkout.order.delete') }}",
                method:"POST",
    ...
    $('.rating').on('click', 'li', function(){
        ...
        $.ajax({
            url:"{{ route('en.product.review.add', [$product->title_tag]) }}",
            method:"POST",
    ...
    $('.rate-modal').on('click', '.submit-review', function(){
        ...
        $.ajax({
            url:"{{ route('en.product.review.save', [$product->title_tag]) }}",
            method:"POST",
    ...
    $('.customers-reviews').on('click', '.helpful-button', function(){
        ...
        $.ajax({
            url:"{{ route('en.product.review.helpful', [$product->title_tag]) }}",
            method:"POST",
    ...
    $('.customers-reviews').on('click', '.not-helpful-button', function(){
        ...
        $.ajax({
            url:"{{ route('en.product.review.nothelpful', [$product->title_tag]) }}",
            method:"POST",
            data:{_token: CSRF_TOKEN, review_id:review_id},
            dataType:'JSON',
            beforeSend: function(){
