...
<html lang="en">
    ...
    <body data-spy="scroll" data-target="#header" data-offset="80">
        <!--<section id="container">-->
            @include('tashtebk.english.layouts.navigationbar')

                @yield('content')

            @include('tashtebk.english.layouts.footer')
        <!--</section>-->
       ...
       <!--</div>-->
        ...
        <script>
            ...
            $( " .notification-count " ).on('click', function(){
                var CSRF_TOKEN = $('meta[name="csrf-token"]').attr('content');
                $.ajax({
                    url:"{{ route('en.notification.clear') }}",
                    method:"POST",
                    ...
                    success: function(data)
                    {
                        ...
                        $('.notification-tag').html('You have 0 new notifications.');
                    }
                });
